...
- 2026-09-17 **板内事件日志上线（commit 33f17a3，用户需求=状态转换/上报内容可回看，暂存+上传 GPU 机）**：rig_ev 48 槽 PSRAM 环形缓冲（drop-oldest）→ 成功轮询后批量 POST /beacon → 落 `/opt/rig-stats/logs/events.jsonl`（200 才提交，失败下轮重试）。埋点 13 事件：boot(含复位原因)/wake_lowbatt（**RTC 标记跨深睡，深睡原因下次开机补报**）/ip/wrong_subnet/wifi_dc/cad 升降档/ls armed·gated/ps max·min/page 翻页/lowbatt 首越线/deep_sleep/offline/online。验证=开机三事件单批上传。**日志体系四层定型**：events.jsonl（状态转换史）+ board.jsonl（每拍快照：电压/节拍/RSSI）+ USB 控制台（细节+panic）+ Mac 哨兵（关键行）。查询：`ssh cui@192.168.1.12 'tail -50 /opt/rig-stats/logs/events.jsonl'`。
