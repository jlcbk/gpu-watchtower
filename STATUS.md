...
- 2026-09-17 **电池首日实测（用户装入 18650）**：全天 4.13V→3.88V；分窗斜率=闲时 -21mV/h（0% busy）/忙时 -23mV/h（77% busy）——节拍设计有效（忙闲仅差 10%）但闲时 21mV/h 远超 5-10mA 目标（折算 20-30mA）；tickless 编译已确认生效，嫌疑收敛=①每次轮询新建 TCP 连接（keep-alive 待做，最大已知固件侧浪费）②板级外围静态耗电（codec/传感器未休眠）。测量方法论=**电压斜率当电流表**（同电芯同工况 A/B 对比）。
